...
- [x] Source health watch (2026-09-25): `status.json` per market run,
...
- [x] Localisation (2026-09-25): 146 user-visible resources externalised
      (values/strings.xml + strings_model / strings_feed / strings_detail),
      ViewModel status messages carry resource ids, category / date-filter /
      source-action labels carry @StringRes; per-app language config
      (xml/locales_config.xml, AGP localeFilters) for en, es, fr, pt, ht, nl.
      Translations generated by `zero_fomo/tools/translate_strings.py` on the
      FIE flash tier (DeepSeek), validated for names / placeholders / plurals;
      first drafts — a native read-through per language is still owed before
      the Play listing is localised. Feed content stays in its source
      language. Next: localise the Play store listing and the share-target
      label, add `values-es-rMX` / `values-pt-rBR` only if regional wording
      diverges, and let the market's country pick a default language hint.

- [x] Caribbean rollout (2026-09-25): 22 new market files (OECS, ABC islands,
