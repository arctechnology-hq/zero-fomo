...
param(
    [Parameter(Mandatory)][ValidateSet('discord', 'telegram', 'instagram')][string]$Bridge,
    [string]$Token,
    [string]$Map,
    [string]$IgUserId,
...
$spec = @{
    discord   = @{ unit = 'zerofomo-discord';   token = 'DISCORD_BOT_TOKEN';  map = 'DISCORD_CHANNEL_MARKETS' }
    telegram  = @{ unit = 'zerofomo-telegram';  token = 'TELEGRAM_BOT_TOKEN'; map = 'TELEGRAM_DEFAULT_MARKET' }
    instagram = @{ unit = 'zerofomo-instagram'; token = 'IG_ACCESS_TOKEN';    map = 'IG_HASHTAGS' }
}[$Bridge]
...
if grep -q "^$($spec.token)=.\+" "`$ENV"; then
  ...
  echo "$($spec.token) still empty on the node; unit left staged"
fi
